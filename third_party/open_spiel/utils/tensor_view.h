...
class TensorView {
 public:
  TensorView(std::vector<double>* values, const std::array<int, Rank>& shape,
             bool reset)
  ...
  int index(const std::array<int, Rank>& args) const {
    int ind = 0;
  ...
  double& operator[](const std::array<int, Rank>& args) {
    return (*values_)[index(args)];
  }
  double& operator[](const std::array<int, Rank>& args) const {
    return (*values_)[index(args)];
  }
