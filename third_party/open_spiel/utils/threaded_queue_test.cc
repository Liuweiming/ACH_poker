...
void TestThreadedQueue() {
  ...
  SPIEL_CHECK_FALSE(q.Pop(std::chrono::milliseconds(1)));
  SPIEL_CHECK_FALSE(q.Pop(
      std::chrono::time_point_cast<Duration>(std::chrono::system_clock::now()) +
      std::chrono::milliseconds(1)));

  SPIEL_CHECK_TRUE(q.Push(10, std::chrono::time_point_cast<Duration>(
  ...
  SPIEL_CHECK_FALSE(q.Push(15, std::chrono::milliseconds(1)));

  CheckPopEq(11);

  SPIEL_CHECK_TRUE(q.Push(16, std::chrono::milliseconds(1)));
...
int main(int argc, char** argv) { open_spiel::TestThreadedQueue(); }
