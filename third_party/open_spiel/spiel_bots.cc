...
std::unique_ptr<Bot> MakeUniformRandomBot(Player player_id, int seed) {
  return std::unique_ptr<UniformRandomBot>(
      new UniformRandomBot(player_id, seed));
}

// A bot that samples from a policy.
std::unique_ptr<Bot> MakePolicyBot(const Game& game, Player player_id, int seed,
                                   std::unique_ptr<Policy> policy) {
  return std::unique_ptr<PolicyBot>(new PolicyBot(seed, std::move(policy)));
}
...
std::unique_ptr<Bot> MakeFixedActionPreferenceBot(
  ...
  return std::unique_ptr<FixedActionPreferenceBot>(
      new FixedActionPreferenceBot(player_id, actions));
}

std::unique_ptr<Bot> MakeStatefulRandomBot(const Game& game, Player player_id,
                                           int seed) {
  return std::unique_ptr<StatefulRandomBot>(
      new StatefulRandomBot(game, player_id, seed));
}
