...
class TabularPolicy : public Policy {
 public:
  ...
  TabularPolicy(const Game& game, const Policy& policy) : TabularPolicy(game) {
    for (auto& inf : policy_table_) {
      inf.second = policy.GetStatePolicy(inf.first);
    }
  }
...
class FristPolicy : public Policy {
 public:
  ActionsAndProbs GetStatePolicy(const State& state) const {
    ActionsAndProbs probs;
    std::vector<Action> actions = state.LegalActions();
    ...
    for (int i = 0; i != actions.size(); ++i) {
      probs.push_back({actions[i], i == 0 ? 1.0 : 0.0});
    }
    return probs;
  }
};
...
class RandomPolicy : public Policy {
 public:
  RandomPolicy(std::mt19937* gen, std::uniform_real_distribution<double>* dist)
      : gen_(gen),
        dist_(dist),
        policy_table_(new std::unordered_map<std::string, ActionsAndProbs>) {}
  ~RandomPolicy() { delete policy_table_; }
  ActionsAndProbs GetStatePolicy(const State& state) const {
    std::string info_str = state.InformationStateString();
    if (policy_table_->find(info_str) != policy_table_->end()) {
      return (*policy_table_)[info_str];
    }
    ActionsAndProbs probs;
    std::vector<Action> actions = state.LegalActions();
    probs.reserve(actions.size());
    double sum = 0;
    for (int i = 0; i != actions.size(); ++i) {
      double prob = (*dist_)(*gen_);
      sum += prob;
      probs.push_back({actions[i], prob});
    }
    for (auto& action_and_prob : probs) {
      action_and_prob.second /= sum;
    }
    policy_table_->insert({info_str, probs});
    return probs;
  }
