...
Suit CardSuit(int card) { return static_cast<Suit>(card / 8); }

Rank CardRank(int card) { return static_cast<Rank>(card % 8); }
...
SkatState::SkatState(std::shared_ptr<const Game> game) : State(game) {
...
SkatGame::SkatGame(const GameParameters& params) : Game(kGameType, params) {}
