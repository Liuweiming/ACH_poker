...
constexpr double kLossUtility = -1;
constexpr double kWinUtility = 1;
constexpr int kCellStates = 3;

// Go can only end in a draw when using a round komi.
// We also treat superko as a draw.
constexpr double kDrawUtility = 0;

// All actions must be in [0; NumDistinctActions).
constexpr int kNumDistinctActions = kActionPass + 1;
