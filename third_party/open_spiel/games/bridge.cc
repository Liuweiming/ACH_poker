...
constexpr int kFirstBid = kRedouble + 1;
...
std::string BridgeState::ToString() const {
  std::string rv = absl::StrCat(
      "Vul: ", is_vulnerable_[0] ? (is_vulnerable_[1] ? "All" : "N/S")
...
std::string BridgeState::ObservationString(Player player) const {
  ...
  if (tensor[0] || tensor[1]) {
    ...
    absl::StrAppend(
        &rv, "V:",
        ptr[1] ? (ptr[3] ? "Both" : "We  ") : (ptr[3] ? "They" : "None"), " ");
    ...
    if (num_passes_ == 4) {
      ...
      if (use_double_dummy_result_) {
        ...
        num_declarer_tricks_ =
            double_dummy_results_.resTable[contract_.trumps]
                                          [contract_.declarer];
        ScoreUp();
      } else {
        phase_ = Phase::kPlay;
