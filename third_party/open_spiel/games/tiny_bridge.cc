...
// indistinguishable.
constexpr char* kAbstraction[kNumAbstractHands] = {
    // Mixed suits.
    "SAHA", "SJHA SKHA SQHA", "SAHJ SAHK SAHQ",
    "SJHJ SJHK SJHQ SKHJ SKHK SKHQ SQHJ SQHK SQHQ",
    // Hearts only.
    "HAHK HAHQ", "HKHJ HKHQ", "HAHJ", "HQHJ",
    // Spades only.
    "SASK SASQ", "SKSQ SKSJ", "SASJ", "SQSJ",
};
...
std::string TinyBridgeAuctionState::ObservationString(Player player) const {
  ...
  if (HasAuctionStarted()) {
    ...
    if (state.redoubler != kInvalidSeat)
      absl::StrAppend(
          &observation, " ", "RDbl:",
          RelativeSeatString(state.redoubler, PlayerToSeat(player)));
  }
  return observation;
}
