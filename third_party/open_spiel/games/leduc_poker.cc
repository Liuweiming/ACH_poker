...
std::string LeducState::ToString() const {
  std::string sequence;
  for (auto a : round1_sequence_) {
    sequence.push_back(ActionToString(0, a)[0]);
  }
  for (auto a : round2_sequence_) {
    sequence.push_back(ActionToString(0, a)[0]);
  }
  std::string private_cards = absl::StrJoin(private_cards_, "|");
  return absl::StrFormat(
      "%s:%s:%s", private_cards,
      public_card_ >= 0 ? std::to_string(public_card_) : "-1", sequence);
}
...
std::unique_ptr<State> LeducState::ResampleFromInfostate(
  ...
  std::unique_ptr<LeducState> clone =
      std::unique_ptr<LeducState>(new LeducState(game_));
