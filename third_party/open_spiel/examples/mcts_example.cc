...
int main(int argc, char** argv) {
  ...
  for (int game_num = 0; game_num < num_games; ++game_num) {
    auto return_and_history = PlayGame(*game, bots, rng, initial_actions);
    histories[absl::StrJoin(return_and_history.second, " ")] += 1;
    for (int i = 0; i < return_and_history.first.size(); ++i) {
      double v = return_and_history.first[i];
      overall_returns[i] += v;
      if (v > 0) {
        overall_wins[i] += 1;
      }
