...
std::unique_ptr<open_spiel::Bot> MakeBot(
  ...
  return std::unique_ptr<open_spiel::algorithms::MCTSBot>(
      new open_spiel::algorithms::MCTSBot(
...
int main(int argc, char** argv) {
  ...
  std::map<std::string, std::function<std::string(const Args&)>> cmds = {
      ...
      {"game",
       [&bot, &game, &state, &evaluator](const Args& args) {
         if (args.empty()) {
      ...
      {"clear_board",
       [&bot, &game, &state](const Args& args) {
         state = game->NewInitialState();
         bot->Restart();
         return Success();
       }},
      {"undo",
       [&bot, &game, &state](const Args& args) {
         ...
         if (history.size() < count) {
           return Failure(absl::StrCat("Can't undo ", count,
                                       " moves from game of length ",
                                       history.size()));
         }
         state = game->NewInitialState();
         bot->Restart();
      ...
      {"showboard",
       [&state](const Args& args) {
         return Success("\n" + state->ToString());
       }},
      {"history",
       [&state](const Args& args) { return Success(state->HistoryString()); }},
      {"is_terminal",
       [&state](const Args& args) {
