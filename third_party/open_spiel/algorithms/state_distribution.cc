...
std::unique_ptr<HistoryDistribution> UpdateIncrementalStateDistribution(
  ...
  if (previous == nullptr)
    previous = std::unique_ptr<HistoryDistribution>(new HistoryDistribution);
  if (previous->first.empty()) {
    // If the previous pair is empty, then we have to do a BFS to find all
    // relevant nodes:
    return std::unique_ptr<HistoryDistribution>(
        new HistoryDistribution(GetStateDistribution(state, opponent_policy)));
  }
