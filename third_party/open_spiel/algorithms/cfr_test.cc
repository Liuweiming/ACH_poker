...
void CFRTest_CFRInfoStateValues() {
  CFRInfoStateValues value({0, 1, 2}, 0.001);
  SPIEL_CHECK_EQ(std::accumulate(value.current_policy.begin(),
                                 value.current_policy.end(), 0.0),
                 1.0);
  // Regret Match
  value.SetRegret(std::vector<double>{0.1, -0.2, 0.4});
  value.ApplyRegretMatching();
  std::vector<double> policy{0.2, 0, 0.8};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_EQ(value.current_policy[i], policy[i]);
  }
  // Regret Match Using MAX
  value.ApplyRegretMatchingUsingMax();
  policy = std::vector<double>{0.2, 0, 0.8};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_TRUE(Near(value.current_policy[i], policy[i]));
  }
  value.SetRegret(std::vector<double>{-0.1, -0.2, -0.4});
  value.ApplyRegretMatchingUsingMax();
  policy = std::vector<double>{1, 0, 0};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_TRUE(Near(value.current_policy[i], policy[i]));
  }
  value.SetRegret(std::vector<double>{-0.1, 0, -0.4});
  value.ApplyRegretMatchingUsingMax();
  policy = std::vector<double>{0, 1, 0};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_TRUE(Near(value.current_policy[i], policy[i]));
  }
  // Epsilon Greedy
  value.SetRegret(std::vector<double>{0.1, -0.2, 0.4});
  value.ApplyEpsilonGreedy(0.3);
  policy = std::vector<double>{0.1, 0.1, 0.8};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_TRUE(Near(value.current_policy[i], policy[i]));
  }
  // Set Policy
  value.SetPolicy(std::vector<double>{0.1, 0.5, 0.4});
  policy = std::vector<double>{0.1, 0.5, 0.4};
  for (int i = 0; i != value.num_actions(); ++i) {
    SPIEL_CHECK_TRUE(Near(value.current_policy[i], policy[i]));
  }
}
...
int main(int argc, char** argv) {
  algorithms::CFRTest_CFRInfoStateValues();
