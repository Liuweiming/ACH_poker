...
Trajectory PlayGame(Logger* logger, int game_num, const open_spiel::Game& game,
  ...
  while (true) {
    ...
    trajectory.states.push_back(ReplayNode{state->ObservationTensor(), player,
                                           state->LegalActions(), action,
      ...
      for (int i = 0; i < replay_buffer.Size() / config.train_batch_size; i++) {
        ...
        for (const ReplayNode& node : data) {
          train_inputs.emplace_back(
              VPNetModel::TrainInputs{node.legal_actions, node.observation,
                                      node.policy, node.returns[0]});
        }
