...
void actor(const Game& game, const DeepCFRConfig& config, int num,
  ...
  ACHSolver solver(game, value_0_eval, value_1_eval, policy_0_eval,
                   policy_1_eval, config.use_regret_net, config.use_policy_net,
                   config.use_tabular, config.nfsp_anticipatory, 1e-5,
...
void preprocess_trajectory(const DeepCFRConfig& config, Player player,
  ...
  for (++iter; iter != trajectory->states.end(); ++iter, ++next) {
    ...
    double importance = iter->current_policy / iter->old_policy;
    cum_reward = importance *
                 (next->reward * config.ach_reward_scale + gamma * cum_reward);
    double delta = next->reward * config.ach_reward_scale +
                   (gamma * next->value) - iter->value;
    double adv = delta + (gamma * lambda * last_importance * next->adv);
    ...
    for (int train_step = 0; train_step < train_steps; train_step++) {
      ...
      if (value_or_policy) {
        ...
        for (int i = 0; i != data.size(); ++i) {
          ...
          std::vector<double> target_value(3, 0);
          target_value[0] = node.adv;           // adv
          target_value[1] = node.value_target;  // value
          target_value[2] = node.old_policy;    // policy
...
bool deep_cfr(DeepCFRConfig config, StopToken* stop) {
  ...
  if (config.graph_def.empty()) {
    config.graph_def = absl::StrJoin({config.game, std::string("ach")}, "_");
    ...
    CreateModel(*game, config.learning_rate, config.weight_decay, model_path,
                config.graph_def, "ach", "ach", config.nn_width,
  ...
  for (int i = 0; i < config.num_gpus; ++i) {
    device_manager.AddDevice(CFRNetModel(
        *game, config.path, model_path, config.graph_def, config.omp_threads,
        ...
        config.use_policy_net, true, true, true));
    if (config.num_cpus) {
