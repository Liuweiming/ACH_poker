...
ACHSolver::ACHSolver(const Game& game,
...
std::pair<Trajectory, Trajectory> ACHSolver::RunIteration(Player player,
                                                          double alpha,
                                                          int step) {
  return RunIteration(rng_, player, alpha, step);
}

std::pair<Trajectory, Trajectory> ACHSolver::RunIteration(std::mt19937* rng,
                                                          Player player,
                                                          double alpha,
                                                          int step) {
  alpha_ = alpha;
  ...
  current_or_average[player] = eta_ != 0;
...
double ACHSolver::UpdateRegrets(
  ...
  if (step != 1) {
    ...
    current_value = inf_output.back();
    inf_output.pop_back();
    SPIEL_CHECK_EQ(inf_output.size(), legal_actions.size());
    current_info_state.SetRegret(inf_output);
  }
  current_info_state.ApplyHedge(1.0);
  CFRInfoStateValues sampling_info_state = current_info_state;
  if (cur_player == player && !current_or_average[player]) {
    std::vector<double> up(legal_actions.size(), 1.0 / legal_actions.size());
    sampling_info_state.SetPolicy(up);
  }
  double value = 0;
  int aidx = 0;
  if (cur_player == player) {
    aidx = sampling_info_state.SampleActionIndex(0.0, dist_(*rng));
    double new_reach = current_info_state.current_policy[aidx] * player_reach;
    double new_sampling_reach =
        sampling_info_state.current_policy[aidx] * sampling_reach;
    value = UpdateRegrets(
  ...
  if (cur_player == player) {
    value_trajectory.states.push_back(ReplayNode{
        ...
        current_value, 1.0, sampling_info_state.current_policy[aidx],
        current_info_state.current_policy[aidx], 0.0});
